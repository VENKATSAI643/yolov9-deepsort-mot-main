...
def run(
    ...
    # Directories
    save_dir = increment_path(Path(ROOT) / 'results' / name, exist_ok=exist_ok)  # increment run
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov9-c.pt', help='model path or triton URL')
    parser.add_argument('--source', type=str, default=ROOT / 'uploads/test.mp4', help='file/dir/URL/glob/screen/0(webcam)')
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        # Save the file as 'test.mp4'
        filename = 'test.mp4'
        input_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        print(f"Uploading file and saving as: {filename}")
        file.save(input_path)

        # Call the main function (assuming it processes the file and saves results in an exp folder)
        opt = parse_opt()
        main(opt)
        ...
        if video_files:
            ...
            print(f"Processing completed. Video saved at {latest_video}")
            return redirect(url_for('uploaded_file', folder=os.path.basename(latest_exp_folder), filename=video_filename))
        else:
            print("No video file found in the latest experiment folder.")
            return "No video file found in the latest experiment folder."
    else:
...
def serve_video(folder, filename):
    video_path = os.path.join(app.config['PROCESSED_FOLDER'], folder, filename)
